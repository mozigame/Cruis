...
public class CashbackReqRedisServiceImpl implements CashbackReqRedisService {
    ...
    public boolean save(CashbackReq req) {
        try {
            ...
            String key = RedisConstants.CLEAR_PREFIX.PLUTUS_CAHSBACK.key(DateUtil.formatDateTime(new Date(req.getProduceTime()), "yyMMddHH"));
    ...
    public List<CashbackReq> batchPop(Date date) {
        try {
            Jedis jedis = criusJedisFactory.getInstance();
            String key = RedisConstants.CLEAR_PREFIX.PLUTUS_CAHSBACK.key(DateUtil.formatDateTime(date, "yyMMddHH"));
